...
const DEFAULT_HEADER_NAME = "x-feature-flags";

function getHeaderName() {
  return process.env.NMF_HEADER_NAME || DEFAULT_HEADER_NAME;
}
...
export function attachFlagsToResponseHeaders(req, res) {
  ...
  res.headers.set(getHeaderName(), encoded);
   if (shouldDebug(process.env.NODE_ENV)) {
    const enabled = Object.entries(flags)
      .filter(([, v]) => v === true)
      .map(([k]) => k);

    console.log("[next-micro-flags] evaluated", {
      userId: userId || null,
      visitorId: visitorId || null,
      rolloutKey: rolloutKey || null,
...
export function getFlagsFromHeaders(headers) {
  const encoded = headers.get(getHeaderName());
  if (!encoded) return {};
...
export const FEATURE_FLAGS_HEADER = DEFAULT_HEADER_NAME;
