...
export function evaluateFlag(
  flagName,
  { userId, rolloutKey, env = process.env.NODE_ENV } = {}
) {
...
export function evaluateAllFlagsForUser({
  ...
  for (const flagName of Object.keys(featureFlagsConfig)) {
    result[flagName] = evaluateFlag(flagName, { userId, rolloutKey, env });
  }
  return result;
}
